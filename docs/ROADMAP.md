...
Shear's first rules overlap known autofixers. The competitive question is not "are the rules novel" but "what can Shear do that current tools cannot, and is that worth a separate step in someone's workflow." This document owns the forward-looking direction; [ARCHITECTURE.md](ARCHITECTURE.md) owns the stable design.

## Verified differentiation

Measured against Ruff 0.16.6 and ESLint 10.10.0 on 2026-09-09 (workstation versions; re-verify before relying on any claim):
...
Ruff's newer versions keep adding unsafe fixes, so each capability row needs periodic re-verification against the current release.
...
**Convergence on combined passes.** Shear composes rules to a fixed point, so `redundant-else` → `merge-nested-if` → `guard-clause` chains apply automatically. Ruff's one-shot per-rule fix model cannot express this class without separate runs. Verified as designed behavior; the value claim rests on the rules themselves being worth running.
...
The old evidence machine (pinned-revision demo scripts, giant workspace copies) is retired. Replace it with a lightweight pattern: fresh clone → `shear` → before/after check runs → inspect diff, without hash-pin scripts. A new pass over CPython and two JS projects with the current binary establishes what the tool does today and feeds honest numbers into any comparison claim.

### 5. The trusted-step experience

Agents already run formatters; Shear's premise is being runnable the same way — deterministic, offline, no model, bounded runtime. The verified differentiators there: whole-file reparse after every edit, comment-multiset preservation enforced at every rewrite, and refusal-to-write on any failed check. These are not roadmap items; they are existing guarantees to keep and state clearly.

## Known output-quality gaps

- **Python guard negation:** `not (expr)` where `not expr` is idiomatic and precedence-safe (priority 1a).
- **JavaScript nested-if merge:** the merged header always parenthesizes both operands, producing `if ((a) && (b))` even for simple identifiers. Python's `and_operand` already applies precedence-aware rendering; the JS backend should match it.

## Non-goals

Helper extraction, renaming, arbitrary restructuring, "AI-suggested" rewrites, type checking, a universal semantic AST, a generic codemod framework, performance work beyond the 2 MiB / 1,024-edit bounds, additional languages before current backends justify them, and optimizing any complexity score.

## Milestones

| # | Milestone | Exit criteria |
|---|---|---|
| 1 | Idiomatic guard output | `not (x)` → `not x` where safe; JS merge operands rendered without gratuitous parens; fixtures for precedence cases; README example updated |
...
| 6 | Release decision | With 1–4 done, revisit the license question and distribution with the user |
...
Keep the engine's guarantees intact: any new rewrite class ships with fixture coverage and a real-target pass, and existing guarantees (reparse, convergence, comment multiset, refusal on failed check) are never traded for coverage.

Learn from Ruff's rule/semantic infrastructure externally; embed specific internals only for a concrete benefit. Ruff's Rust interfaces are unstable — a decision to borrow must survive a version bump.
